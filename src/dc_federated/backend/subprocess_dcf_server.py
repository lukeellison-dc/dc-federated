...
class DCFServer(object):
    ...
    def add_and_register_worker(self):
        ...
        if not self.worker_manager.is_worker_registered(worker_id):
            ...
            # logger.info(f'TEST: {self.context}')
            # # context = zmq.Context()
            # socket = self.context.socket(zmq.REQ)
            # socket.connect(f"tcp://localhost:5555")
            # logger.info(f'new socket created. Sending test mesage')
            # socket.send_multipart([b'server_args_request'])
            # test = socket.recv_pyobj()
            # logger.info(f'TEST OUT: {test}')
    ...
    def receive_worker_update(self, worker_id):
        ...
        try:
            ...
            # print('start process')
            # p = Process(target=self.receive_worker_update_callback, args=(worker_id, model_update))
            # p.start()
...
# logger.info('soocket intitalised')

zqmi = ZQMInterfaceServer(socket=None)
server_subprocess_args = zqmi.server_args_request_send()
logger.info(f'server_subprocess_args received:\n{server_subprocess_args}')
...
logger.info('server intitalised')
zqmi.register_worker_send('test_id')
# time.sleep(10)
zqmi.register_worker_send('test_id2')
server.start_server()
print('test end')
