...
class ZMQInterfaceModel():
    ...
    def receive(self):
        ...
        # Federated Learning API
            ...
            status = self.receive_worker_update_callback(message[1].decode('utf-8'), message[2])
...
class ZMQInterfaceServer():
    ...
    def is_global_model_most_recent_send(self, model_version):
        socket = self._send([b'is_global_model_most_recent', str(model_version).encode('utf-8')])
        return socket.recv_pyobj()

    def receive_worker_update_send(self, worker_id, model_update):
        socket = self._send([b'receive_worker_update', worker_id.encode('utf-8'), model_update])
